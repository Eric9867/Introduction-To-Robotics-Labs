...
class Controller(object):
    ...
    def camera_callback(self, msg):
        ...
        if (abs(self.error_x) > 100):
            self.v = 0.75*self.max_v # or you could proportionally decrease the velocity based on self.error_x
        else:
            self.v=self.max_v
    ...
    def follow_the_line_bang_bang(self):
        # parameters v = 0.075, omega = 0.3
        while not rospy.is_shutdown():
            #print(self.cur_line_x)
            if self.error_x == 0:
                omega = 0
            elif self.error_x < 0:
                omega = self.omega_0
